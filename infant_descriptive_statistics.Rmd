...
p <- data_dyad_total_zeros %>% 
  ...
  geom_point()+
  geom_line() +
  facet_grid(cols=vars(Narrow_group)) +
...
p <- data_dyad_total_zeros %>% 
  ...
  geom_line() +
  facet_grid(cols=vars(Narrow_group)) +
  ggtitle("Overall cue frequencies") +
...
data_dyad_total_zeros %>%
  ...
  ggtitle("Potent behhavior frequency distributions by age",
          subtitle = "Outliers above 2 shown below") + 
  xlab("Cues per minute")# +
  #xlim(0,outliers_over)
...
#   mutate(Behavior = reorder(Behavior, num_per_min)) %>% 
#   filter(num_per_min > 2) %>% 
#   ggplot(aes(y=Behavior, x=num_per_min, col=dyad_set, group=dyad_set)) +
# 
#   geom_point(alpha = 0.75, size=2, position=position_dodge(width=0.75))+
#   ggtitle("Outliers from previous 2 charts (above 2)") +
#   xlim(2, NA)
```
