...
EXCLUDING "State stop" lines currently. They made redundant columns, but may be useful later.
...
# Step 1: Pivot wider to have one column per behavior label
data_onehot <- data_duration %>% 
  ...
  mutate(
    behavior_name = str_c(if_else(Event_Type == "State point" | Event_Type == "State start", "", "Stops "),
                               Behavior)) %>% 
  pivot_wider(names_from =  c(behavior_name), values_from = temp_1, values_fill = 0) 
  
# Step 2: Identify the behavior columns. They must be to the right of all other columns for this method to work, and start with "Eys Open".
onehot_behavior_columns = colnames(data_onehot)
#match finds the index of the first behavior, which is always "Eyes open". This selects that and all remaining column names.
onehot_behavior_columns = onehot_behavior_columns[match("Eyes Open", onehot_behavior_columns):length(onehot_behavior_columns)]
  
# Step 3: Then we group all cues that happen at the same time into one line.
...
group_vec = as.numeric(nrow(df_clusters))
...
write.csv(df_behavior_clusters, "df_behavior_clustsers.csv")
```
