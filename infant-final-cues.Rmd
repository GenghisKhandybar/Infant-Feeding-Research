...
grid_plot <- function(group, num_splits){
  #Similar to before, but not only the first 3 quarters
  n_dyads_cue <- data_duration %>% #Cue representation in dyads before the requested time period
    filter(dyad_unit == "Baby") %>% 
    filter(Grouped_behavior == "Infant Satiation Cues") %>%  
    mutate(split = get_time_split(start_fraction, num_splits)) %>% 
...
#   ggtitle("Individual infants' proportions of final cues (observed at 4 ages)") +
#   xlab("Number of feedings ending in cue") +
#   ylab("Proportion of babies")
# 
# final_cue_count_props %>% 
#   ggplot(aes(x=n, y=cum_prop)) +
#   geom_bar(stat = "identity") +
#   facet_grid(cols=vars(Behavior)) +
#   theme(strip.text.x = element_text(angle=90)) +
#   ggtitle("Individual infants' cumulative proportions of final cues (observed at 4 ages)") +
#   xlab("Number of feedings ending in cue") +
#   ylab("Proportion ending in cue n or more times") 
```
