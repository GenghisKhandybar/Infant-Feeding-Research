---
title: "Two-Month Bottle Feeding: Data Processing"
author: "Kevin Ross"
date: "`r Sys.Date()`"
...
raw_data <- list.files(path = "./2 month bottle-feeding data", recursive = TRUE, pattern = "*.xlsx", full.names = TRUE) %>% 
  map_df(~ read_sheets(.))

data <- raw_data %>%
  mutate(dyad_unit = ifelse(str_detect(`Excel_file_name`,"baby"), "Baby", "Mom")) %>%
  select(-Excel_file_name) %>%
  mutate(Observation = str_remove(Observation," Part 1"), # bit of data cleaning
         Observation = str_remove(Observation," Part 2"),
         Observation = str_remove(Observation," Part 3"),
         Observation = str_remove(Observation,"(p1)"),
         Observation = str_remove(Observation,"(p2)"),
         Observation = str_remove(Observation,"(p3)"),
         Observation = str_remove(Observation," pt1"),
         Observation = str_remove(Observation," pt2"),
         Observation = str_remove(Observation," pt3"),
         Observation = str_remove(Observation,"\\(\\)"),
...
bottle_rej_2mo <- read_excel("Bottle Rejectors.xlsx",sheet = 2) %>%
  filter(`Rejected? (1=yes, 0=no, 99=not coded)` == 1)

data <- data %>%
  mutate(Bottle_Rejector = ifelse(ID_Number %in% bottle_rej_2mo$id, 1,0))
```
...
write.csv(data, "./data-2moB.csv", row.names = FALSE)
```
