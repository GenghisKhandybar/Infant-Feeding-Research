...
baby_4mo <- read_excel("4-month Bottle-Feeding Coding_FINAL BABY DATA EXPORT.xlsx") %>%
  mutate(dyad_unit = "Baby",
         Modifier_1 = NA)
mom_4mo <- read_excel("4-month Bottle-Feeding Coding_FINAL MOM DATA EXPORT.xlsx") %>%
  mutate(dyad_unit = "Mom",
         Comment = NA)

# Combine the Data
# - remove first 7 columns as they are not needed
# - create dyad_set variable
# create time and duration
# arrange by ID and time
# replace spaces with underscores in variable names

data <- rbind(baby_4mo, mom_4mo) %>%
  select(-(1:7))%>%
  mutate(dyad_set = "4moB") %>%
...
rm(baby_4mo, mom_4mo, add_time_zero_start, add_time_zero_end) 
```

```{r}
# adding the bottle rejectors
bottle_rej_4mo <- read_excel("Bottle Rejectors.xlsx",sheet = 3) %>%
  filter(`Rejected? (1=yes, 0=no, 99=not coded)` == 1)

data <- data %>%
  mutate(Bottle_Rejector = ifelse(ID_Number %in% bottle_rej_4mo$id, 1,0))
```


# A few checks to make sure data is valid

## Check that each mom starts with "Bottle not in Mouth" (Code should return 0 rows)
```{r}
# check that each  starts with "Bottle not in Mouth" 
data %>%
  filter(dyad_unit == "Mom") %>%
  group_by(ID_Number)%>%
  slice(1) %>%
...
write.csv(data, "./data-4moB.csv", row.names = FALSE)
```