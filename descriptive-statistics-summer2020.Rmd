...
plot_baby_cues_grouped <- data %>%
  filter(dyad_unit == "Baby") %>%
  filter(!(Behavior %in% c("Eyes Open", "Not Crying"))) %>%
  filter(!(Event_Type == "State stop")) %>%
  filter(!is.na(Bottle_state)) %>%
  group_by(dyad_set, Narrow_group, Bottle_state) %>%
  count(name = "count") %>% 
  ggplot(aes(fill = Bottle_state,
         y = count,
         x = Narrow_group)) + 
  geom_bar(position = "fill", stat = "identity") +
  coord_flip() +
...
plot_grid(
  plot_grid(plot_baby_cues_by_bottle_state + theme(legend.position="none"),
            plot_baby_cues_grouped  + theme(legend.position="none"),
...
data_dyad_total_bottle_in_mouth %>% 
  group_by(dyad_set) %>% 
  summary_5_num("num_occurrences") %>% 
  arrange(dyad_set) %>% 
  kable(digits = 2, caption = "Bottle-in-mouth bouts per dyad set")
```
...
bottle_in_mouth_bouts %>% 
  group_by(dyad_set) %>% 
  summary_5_num("bout_duration") %>% 
  arrange(dyad_set) %>% 
  kable(digits = 2, caption = "Duration of bottle-in-mouth bouts")
```
...
bottle_in_mouth_bouts %>% 
  group_by(dyad_set) %>% 
  summary_5_num("num_cues") %>% 
  arrange(dyad_set) %>% 
  kable(digits = 2, caption = "Infant cues during Bottle-in-mouth bouts")
```
...
plot_bottle_in_mouth_pdf <- ggplot(bottle_in_mouth_bouts,
  ...
  ggtitle("Infant cues/min during Bottle-in-mouth bouts")
...
bottle_in_mouth_bouts %>% 
  group_by(dyad_set) %>% 
  summary_5_num("num_cues_by_minute") %>% 
  arrange(dyad_set) %>% 
  kable(digits = 2, caption = "Infant cues per minute during Bottle-in-mouth bouts")
```
...
bottle_in_mouth_bouts %>% 
  ...
  summary_5_num("num_cues") %>% 
  arrange(dyad_set) %>% 
  kable(digits = 2, caption = "Infant cues during last 25% of duration of Bottle-in-mouth bouts")
```
...
bottle_in_mouth_bouts_last_cue %>% 
  group_by(dyad_set, cue_at_time) %>% 
  summarise(count = n()) %>% 
  group_by(dyad_set) %>% 
  mutate(total = sum(count),
         proportion = count / total) %>% 
  ungroup() %>% 
  arrange(cue_at_time, dyad_set) %>% 
  select(dyad_set, cue_at_time, proportion, count, total) %>% 
  kable(digits = 3, caption = "Last infant cue during bottle-in-mouth bouts")
```
...
data_time_event %>% 
  ...
  arrange(cue_at_time, dyad_set) %>% 
  select(dyad_set, cue_at_time, proportion, count, total) %>% 
  kable(digits = 3, caption = "Infant cues during bottle-in-mouth bouts")
```
...
bottle_in_mouth_bouts_last_cue %>% 
  group_by(dyad_set) %>% 
  summary_5_num(to_summarise = "time_fraction_bottle_state") %>% 
  arrange(dyad_set) %>% 
  kable(digits = 3, caption = "Proportional time of last infant cue during Bottle-in-mouth bouts")
```
...
bottle_in_mouth_bouts_last_cue %>% 
  group_by(dyad_set) %>% 
  summary_5_num(to_summarise = "time_remaining_in_bottle_state") %>% 
  arrange(dyad_set) %>% 
  kable(digits = 3, caption = "Remaining time in bout after last infant cue during Bottle-in-mouth bouts")
```
...
bottle_in_mouth_bouts_last_cue %>% 
  ...
  summary_5_num(to_summarise = "time_fraction_bottle_state") %>% 
  arrange(cue_at_time, dyad_set) %>% 
  kable(digits = 2, caption = "Time from last cue until end of bout (sec)")
```
...
# final_bottle_not_in_mouth_bouts <- data_time_event %>%
#   filter(Bottle_not_in_mouth==1) %>%
#   group_by(dyad_set, id) %>%
#   slice_max(Bottle_state_counter) %>%
...
final_bottle_in_mouth_bouts %>% 
  group_by(dyad_set) %>% 
  summary_5_num(to_summarise = "num_cues") %>% 
  arrange(dyad_set) %>% 
  kable(digits = 3, caption = "Infant cues during final Bottle-in-mouth bout for each dyad")
```
...
plot_bottle_in_mouth_pdf <- ggplot(final_bottle_in_mouth_bouts,
   ...
   geom_histogram(alpha = 0.5, aes(y = ..density..), position = 'identity') +
  ylab("Density") +
  xlab("Number of infant cues per minute of final bout") +
  ggtitle("Infant cues/min during final Bottle-in-mouth bouts")
...
final_bottle_in_mouth_bouts %>% 
  group_by(dyad_set) %>% 
  summary_5_num(to_summarise = "num_cues_by_minute") %>% 
  arrange(dyad_set) %>% 
  kable(digits = 3, caption = "Infant cues/min during final Bottle-in-mouth bouts")
```
...
final_bottle_in_mouth_bouts_end %>% 
  group_by(dyad_set) %>% 
  summary_5_num(to_summarise = "num_cues") %>% 
  arrange(dyad_set) %>% 
  kable(digits = 3, caption = "Infant cues during last 25% of duration of final Bottle-in-mouth bouts")
```
...
final_bottle_in_mouth_bouts_end %>% 
  group_by(dyad_set) %>% 
  mutate(num_cues_lumped = fct_lump(as.factor(num_cues), 3)) %>% 
  group_by(dyad_set, num_cues_lumped) %>% 
  summarise(count = n()) %>% 
  group_by(dyad_set) %>% 
  mutate(total = sum(count),
         proportion = count / total) %>% 
  select(dyad_set, num_cues_lumped, proportion, count, total) %>% 
  kable(digits = 2, caption = "Infant cues during last 25% of duration of final Bottle-in-mouth bouts")
```
...
final_bottle_in_mouth_bouts_last_cue %>% 
  group_by(dyad_set) %>% 
  summary_5_num("time_fraction_bottle_state") %>% 
  arrange(dyad_set) %>% 
  kable(digits = 2, caption = "Last infant cue during final Bottle-in-mouth bouts")
```
...
final_bottle_in_mouth_bouts_last_cue %>% 
  group_by(dyad_set) %>% 
  summary_5_num("time_remaining_in_bottle_state") %>% 
  arrange(dyad_set) %>% 
  kable(digits = 2, caption = "Last infant cue during final Bottle-in-mouth bouts")
```
...
final_bottle_in_mouth_bouts_last_cue %>% 
  ...
  summary_5_num("time_fraction_bottle_state") %>% 
  arrange(cue_at_time, dyad_set) %>%
  kable(digits = 2, caption = "Time of last cue as fraction of bout length")
```
...
final_bottle_in_mouth_bouts_last_cue %>% 
  ...
  summary_5_num("time_remaining_in_bottle_state") %>% 
  arrange(cue_at_time, dyad_set) %>%
  kable(digits = 2, caption = "Final Bottle-in-mouth bouts")
```
...
ggplot(data_time_event %>%
  ...
  ggtitle("Infant cues during Bottle bouts") +
  facet_grid(~ dyad_set)
