...
data_duration %>% 
  ...
  group_by(id, dyad_set) %>% 
  summarise(bottles_offered=n()) %>% 
  ggplot(aes(x=bottles_offered)) +
  geom_histogram(aes(y=..density..)) +
  facet_grid(rows=vars(dyad_set)) +
  ggtitle("Distribution of times that bottle contents are assessed")
...
```{r}
csp <- behavior_codebook %>% 
  filter(Narrow_group == "Controlling Stimulating Feeding Practices") %>% 
  pull(Behavior)

data_time_event %>% 
  arrange(id, dyad_set, -time) %>% 
  group_by(id, dyad_set, Bottle_state_counter) %>% 
  mutate(after_assessment = min(1,cumsum(Assesses.bottle.contents))) %>% 
  filter(after_assessment == 1) %>% 
  filter(num_cues_at_time > 0) %>% 
  mutate(stimulation = min(1, sum(Verbal.stimulation.of.sucking.or.feeding, Physical.stimulation.of.sucking.or.feeding, Pushes.bottle.into.infant.s.mouth))) %>% 
