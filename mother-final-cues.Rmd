...
# Time-based analsis of mothers' cues

## These are mirroring analyses of infant cues, so not all 
...
last_cue_counts %>% 
  ...
  ggtitle("Mothers' final cue before end of session")+ 
...
p <- last_cue_counts %>% 
  ...
  xlab("Dyad set") +
  ggtitle("Mothers displaying cues at least once in the final 25% of feeding")
...
for(behavior_type in mother_narrow){
  p <- data_dyad_total_zeros %>%
    ...
    ggtitle(paste(behavior_type, "behhavior frequency distributions by age")) + 
...
data_dyad_total_zeros %>%
  ...
  geom_boxplot(outlier.alpha = 0.25, outlier.size = 1) +
  ggtitle("Mothers' behhavior frequency distributions by behavior group") + 
  xlab("Cues per minute")
```
