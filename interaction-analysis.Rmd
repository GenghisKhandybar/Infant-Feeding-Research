...
```{r}
data_onehot <- read_csv("data_onehot.csv")
```
...
(This is the end for now - improved clusters are in progress.)
...
#Finding names of the behavior columns
...
#Step 3: Group behaviors by cluster. add up all behaviors within clusters
df_cluster_sums <- df_clusters %>% 
  group_by(id, dyad_set, cluster_group) %>% 
  summarize_at(onehot_behavior_columns, sum)
